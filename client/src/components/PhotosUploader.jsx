...
export default function PhotosUploader({ addedPhotos, onChange }) {
    ...
    function uploadPhoto(ev) {
        ...
        axios.post('/upload-photos', data).then(response => {
            ...
            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {addedPhotos.length > 0 && addedPhotos.map(link => (
                    <div className="h-32 flex relative" key={link}>
                        <img className="rounded-2xl w-full object-cover" src={link.includes('http') ? link : 'http://localhost:4000/uploads/' + link} alt="image"></img>
                        ...
                        <button onClick={ev => selectAsMainPhoto(ev, link)} className="cursor-pointer absolute bottom-2 right-12 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-2">
                            {link === addedPhotos[0] && (
